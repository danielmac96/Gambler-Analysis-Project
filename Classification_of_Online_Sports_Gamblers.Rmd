...
summarytable1 <- BetData %>%
  group_by(CountryID) %>%
  summarize(n= n()) %>%
  mutate(Percent_of_sample = round(100*(n/sum(n)),2)) %>%
...
kable(BetData_Both_Clus_Summary %>% mutate(across(c(2,4:6), round, 2)),caption = "Both - Cluster Group Summary Stats")
```
...
kable(BetData_Live_Clus_Summary %>% mutate(across(c(2,4:6), round, 2)),caption = "Live - Cluster Group Summary Stats")
```
...
kable(BetData_Fixed_Clus_Summary %>% mutate(across(c(2,4:6), round, 2)),caption = "Fixed - Cluster Group Summary Stats")
```
